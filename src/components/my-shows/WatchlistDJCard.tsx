  ...
  return (
    <div className="rounded-xl overflow-hidden bg-[#1a1a1a] border border-gray-800/50 relative group">
      ...
      <button
        onClick={onRemove}
        disabled={isRemoving}
        className="absolute top-2 right-2 z-10 p-1 rounded-full bg-black/50 transition-colors text-gray-400 hover:text-red-400 hover:bg-black/70 disabled:opacity-50"
      ...
      {/* Photo area - links to DJ profile if available */}
      {djUsername ? (
        <Link href={`/dj/${djUsername}`} className="block">
          {photoContent}
        </Link>
      ) : (
        photoContent
