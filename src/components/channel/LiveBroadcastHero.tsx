...
export function LiveBroadcastHero({ jumpToEarliestShow, initialScheduleDate }: { jumpToEarliestShow?: boolean; initialScheduleDate?: Date } = {}) {
  ...
  const djDescription = currentShow?.liveDjDescription || currentShow?.liveDjBio || null;
  ...
  return (
    <section id="live" className="relative z-10 px-4 pt-6 pb-8">
      <div className="max-w-3xl mx-auto">
        ...
        {/* Status line removed — toggle above hero already shows live/restream status */}
        ...
        <div ref={stickyBarRef} className="bg-black relative">
          <div className="flex items-center gap-2 sm:gap-3 py-2 px-1">
            ...
            <button
              onClick={toggle}
              disabled={!isLive}
              className="w-8 h-8 ml-1 flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex-shrink-0"
            >
              {isLoading ? (
