...
export interface Show {
  ...
  additionalDjUsernames?: string[]; // Additional DJ profile slugs (for multi-DJ shows)
...
export interface UserDocument {
  ...
  djProfile?: {
    ...
    sceneIds?: string[];
    // Sharing consent. Both default true (opted in) — DJs can uncheck on
    // /studio. Only youtubeOptIn is currently enforced (hides archives
    // from the YouTube Render tab); metaOptIn is stored for future use
...
export interface IRLShowData {
  ...
  venueName?: string;     // Venue name to display (free text — venues are not an entity)
