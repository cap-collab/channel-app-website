...
export function buildCandidateInputs(
  ...
  return items.map((item) => {
    ...
    // Discovery MEMBERSHIP (ordering is by the unified score, not by tier). The
    // numeric value is retained only as a membership marker (1-4) for legacy
    // compatibility. Membership rules:
    //  - AFFILIATED (crew/borrow) qualifies at ANY priority — the crew signal
    //    bypasses the featured/high gate (a low crew archive competes on score).
    //  - scene+tempo / top-scene / top-tempo matches still require featured/high
    //    (the weak taste signal stays a high-quality set).
    //  - Un-engaged Intense (very_fast never played) is kept OUT of the top-scene
    //    match (scene-only), so a spiral+Intense archive doesn't ride a scene
    //    match in; it can still enter via affiliation/exact-combo where a tempo
    //    signal is present or intended.
    let discoveryTier: 1 | 2 | 3 | 4 | null = null;
    if (isAffiliated) {
      discoveryTier = 2; // affiliated / same crew / same audience — ANY priority
    } else if (sceneTempoMatch && isHighFeatured(item.priority)) {
