...
function serializeApplication(docId: string, data: Record<string, unknown>): DJApplicationSerialized {
  return {
    ...
    setDuration: data.setDuration as number | undefined,
    locationType: data.locationType as 'home' | 'venue' | undefined,
    ...
    preferredSlots: data.preferredSlots as Array<{ start: number; end: number }> | undefined,
    ...
    onlineRadioShow: data.onlineRadioShow as string | undefined,
...
export async function createApplication(data: DJApplicationFormData): Promise<DJApplicationSerialized> {
  ...
  const docRef = await db.collection(COLLECTION).add({
    ...
    showName: data.showName?.trim() || null,
    setDuration: data.setDuration || null,
