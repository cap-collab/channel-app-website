...
const AUDIO_HOST_ID = 'broadcast-audio-host';
  ...
  const handleTrackSubscribed = useCallback(
    (track: RemoteTrack, publication: RemoteTrackPublication, participant: RemoteParticipant) => {
      ...
      if (track.kind === Track.Kind.Audio) {
        ...
        if (!audioElementRef.current) {
          audioElementRef.current = createHostedAudio(AUDIO_HOST_ID);
          registerAudio('live', audioElementRef.current);
        } else if (!audioElementRef.current.parentNode) {
          attachToAudioHost(audioElementRef.current, AUDIO_HOST_ID);
        }
