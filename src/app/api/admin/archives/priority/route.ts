...
const VALID_PRIORITIES = ['featured', 'high', 'medium', 'low', 'hidden'] as const;
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (!priority || !VALID_PRIORITIES.includes(priority)) {
      return NextResponse.json({ error: 'Invalid priority. Must be featured, high, medium, low, or hidden' }, { status: 400 });
    }
