...
import { sendBroadcast2HourReminderEmail } from '@/lib/email';
...
// This cron job runs every 30 minutes to send reminder emails 2h before broadcast slots
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const windowStart = Timestamp.fromMillis(now + 1.5 * 60 * 60 * 1000); // 1.5 hours from now
    const windowEnd = Timestamp.fromMillis(now + 2.5 * 60 * 60 * 1000);   // 2.5 hours from now

    console.log(`[broadcast-2h-reminder-emails] Window: ${new Date(windowStart.toMillis()).toISOString()} to ${new Date(windowEnd.toMillis()).toISOString()}`);

    // Find scheduled slots starting in ~2 hours
    ...
    console.log(`[broadcast-2h-reminder-emails] Found ${snapshot.docs.length} slots in window`);
    ...
    for (const doc of snapshot.docs) {
      ...
      for (const target of emailTargets) {
        try {
          ...
          console.error(`Error sending 2h reminder to ${target.email}:`, error);
          errors.push(`Error for ${target.email}: ${error}`);
        }
      }

      // Mark slot as 2h reminded (even if some individual sends failed, to avoid spam)
      await doc.ref.update({ reminder2hEmailSentAt: now });
    }

    console.log(`[broadcast-2h-reminder-emails] Sent: ${sentCount}, Skipped: ${skippedCount}, Errors: ${errors.length}`);
    ...
    console.error('Error in broadcast 2h reminder emails cron:', error);
