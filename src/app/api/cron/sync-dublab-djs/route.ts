...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    for (const slug of djPaths) {
      const djName = slug.replace(/-/g, " ").replace(/\b\w/g, (c) => c.toUpperCase());
      const normalizedId = normalizeForId(djName);
    ...
    console.log(`[sync-dublab-djs] Complete: created=${created}, updated=${updated}, skipped=${skipped}, errors=${errors}, usernamesReserved=${usernamesReserved}`);
